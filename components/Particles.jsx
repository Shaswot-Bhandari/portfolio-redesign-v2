...
const Particles = ({
  ...
  enableParallax = false, parallaxStrength = 0.08
}) => {
  ...
  useEffect(() => {
    ...
    const update = t => {
      ...
      if (enableParallax) {
        const targetX = mouseRef.current.y * parallaxStrength;
        const targetY = mouseRef.current.x * parallaxStrength;
        targetRotRef.current.x += (targetX - targetRotRef.current.x) * 0.03;
        targetRotRef.current.y += (targetY - targetRotRef.current.y) * 0.03;
      }
      ...
      if (enableParallax) {
        particles.rotation.x += targetRotRef.current.x;
        particles.rotation.y += targetRotRef.current.y;
      }
  ...
  }, [particleCount, particleSpread, speed, moveParticlesOnHover, particleHoverFactor, alphaParticles, particleBaseSize, sizeRandomness, cameraDistance, disableRotation, pixelRatio, particleColors, enableParallax, parallaxStrength]);
